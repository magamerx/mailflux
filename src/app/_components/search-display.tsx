...
const SearchDisplay = () => {
    ...
    const search = api.account.searchEamils.useMutation();
    ...
    return <div className="p-4 max-h-[calc(100vh-50px)] overflow-y-scroll">
        ...
        {search.data?.hits.length === 0 ? <>
            ...
            <ul className="flex flex-col gap-2">
                {search.data?.hits.map(hit => (
                    <li key={hit.id} className="border list-none rounded-md p-4 hover:bg-gray-100 cursor-pointer transition-all hover:bg-gray-900">
                        <h3 className="text-base font-medium">
                            {hit.document.subject}
                        </h3>
                        <p className="text-sm text-gray-500">
                            To: {hit.document.from}
                        </p>
                        <p className="text-sm text-gray-500">
